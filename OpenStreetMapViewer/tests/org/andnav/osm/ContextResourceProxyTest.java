...
public class ContextResourceProxyTest extends AndroidTestCase {
	...
	public void test_getBitmap_compare_with_default() {
		...
		final Bitmap defaultBitmap = defaultResourceProxy.getBitmap(ResourceProxy.bitmap.person); // FIXME this throws an exception
		...
		final ByteBuffer bb1 = ByteBuffer.allocate(contextBitmap.getWidth() * contextBitmap.getHeight() * 4);
		contextBitmap.copyPixelsToBuffer(bb1);
		final ByteBuffer bb2 = ByteBuffer.allocate(defaultBitmap.getWidth() * defaultBitmap.getHeight() * 4);
