...
public class CloudmadeUtil implements OpenStreetMapTileProviderConstants {
	...
	public static void retrieveCloudmadeKey(final Context aContext) {
		...
		mAndroidId = Settings.Secure.getString(aContext.getContentResolver(),
	 ...
	 * 
	 * @param aKey
	 *            the cloudmade key obtained with a call to {@link retrieveCloudmadeKey(Context)}
	...
	public static String getCloudmadeToken(final String aKey) throws CloudmadeException {
		...
		// TODO see issue 75 - persist key and token - if key hasn't changed then return the
		// previous token
