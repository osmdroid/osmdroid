...
/**
 * Draws a list of {@link OverlayItem} as markers to a map. The item with the lowest index is drawn as last
 * and therefore the 'topmost' marker. It also gets checked for onTap first. This class is generic, because
...
public class ItemizedOverlay<T extends OverlayItem> extends Overlay
{
    ...
    public ItemizedOverlay(final Context ctx, final List<T> aList,
            final OnItemGestureListener<T> aOnItemGestureListener, final ResourceProxy pResourceProxy)
    {
            ...
            final OnItemGestureListener<T> aOnItemGestureListener, final ResourceProxy pResourceProxy)
    {
        ...
        this.mDefaultItem = OverlayItem
                .getDefaultItem(pMarker, pMarkerHotspot, pHotSpotPlace, pResourceProxy);
    ...
    public void draw(final Canvas canvas, final MapView mapView, final boolean shadow)
    {
        ...
        for (int i = limit; i >= 0; i--) {
            ...
            pj.toMapPixels(item.getPoint(), curScreenCoords);

            item.onDrawItem(canvas, curScreenCoords);
        }
        // indicate the place touched with a bullseye
        if (DEBUG_GRAPHICS) {
            if (mTouchPoint != null) {
                canvas.drawCircle(mTouchPoint.x, mTouchPoint.y, 20, bullseyePaint);
            }
        }
    }

    /**
     * Each of these methods performs a item sensitive check. If the item is located its corresponding method
     * is called. The result of the call is returned.
     * 
     * Helper methods are provided so that child classes may more easily override behavior without resorting
     * to overriding the ItemGestureListener methods.
     */
    @Override
    ...
    /**
     * When a content sensitive action is performed the content item needs to be identified. This method does
     * that and then performs the assigned task on that item.
     * 
     * @param event
     * @param mapView
     * @param task
            ...
            final ActiveItem task)
    {
        ...
        for (int i = 0; i < this.mItemList.size(); ++i) {
            ...
            item.getItemBoundingRectangle(markerScreenBounds, curScreenCoords);
