...
/**
 * This will allow a {@link SafeDrawOverlay} that is not HW acceleration compatible to work in a HW
 ...
 * </ol>
 */
public abstract class NonAcceleratedSafeOverlay extends SafeDrawOverlay {
	...
	// We don't need a delegate since drawSafe() already functions as one
	// protected abstract void onDraw(Canvas c, MapView osmv, boolean shadow);

	public NonAcceleratedSafeOverlay(Context ctx) {
		super(ctx);
	}

	public NonAcceleratedSafeOverlay(ResourceProxy pResourceProxy) {
	...
	protected final void draw(Canvas c, MapView osmv, boolean shadow) {
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB && c.isHardwareAccelerated()) {
			...
			super.draw(mBackingCanvas, osmv, shadow);
			c.save();
