...
/**
 * This will allow an {@link Overlay} that is not HW acceleration compatible to work in a HW accelerated
 * MapView. It will create a screen-sized backing Bitmap for the Overlay to draw to and then will draw the
 * Bitmap to the HW accelerated canvas. Due to the extra work, it does not draw the shadow layer. If the
 * Canvas passed into the Overlay is not HW accelerated or if {@link #isUsingBackingBitmap()} returns false
 * then it draws normally (including the shadow layer) without the backing Bitmap. <br/>
 * <br/>
 * TODO:
 * <ol>
 * <li>Implement a flag to determine if the drawing has actually changed. If not, then reuse the last frame's
 * backing bitmap. This will prevent having to re-upload the bitmap texture to GPU.</li>
 * </ol>
 */
public abstract class NonAcceleratedOverlay extends Overlay
{
    ...
    protected final void draw(Canvas c, MapView osmv, boolean shadow)
    {
        ...
        boolean atLeastHoneycomb = Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB;
