    ...
    public void mapTileRequestFailed(final MapTileRequestState pState) {
        ...
        if (mTileNotFoundImage != null) {
            ...
            try {
                ...
                Log.w(IMapView.LOGTAG, "error trapped mapTileRequestFailed/mTileNotFoundImage, handler loop threw an exception, this can usually be ignored." + ex.getMessage());
            }
        } else {
            try {
                for (final Handler handler : mTileRequestCompleteHandlers) {
                    if (handler != null) {
                        try {
                            ...
                            Log.i(IMapView.LOGTAG, "error trapped mapTileRequestFailed, handler.sendEmptyMessage threw an exception " + ex.getMessage());
                        }
                    }
                }
            } catch (Exception ex) {
                Log.w(IMapView.LOGTAG, "error trapped mapTileRequestFailed, handler loop threw an exception, this can usually be ignored." + ex.getMessage());
            }
        }
        if (Configuration.getInstance().isDebugTileProviders()) {
